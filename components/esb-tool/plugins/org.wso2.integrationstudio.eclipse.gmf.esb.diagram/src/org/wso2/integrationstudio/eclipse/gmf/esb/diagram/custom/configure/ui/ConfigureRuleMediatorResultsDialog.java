...
public class ConfigureRuleMediatorResultsDialog extends TitleAreaDialog {
    ...
    public ConfigureRuleMediatorResultsDialog(Shell parentShell, RuleMediator ruleMediator,
    ...
    private void unbindResult(int itemIndex) {
        ...
        if (null != result.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, ruleMediator.getResultsConfiguration(),
    ...
    protected void okPressed() {
        for (TableItem item : tblResults.getItems()) {
            ...
            if (!result.getResultName().equals(wrapper.getResultName())) {
                setCommand = new SetCommand(editingDomain, result, EsbPackage.Literals.RULE_RESULT__RESULT_NAME,
                        wrapper.getResultName());
                getResultCommand().append(setCommand);
            }

            if (!(result.getResultType().equals(wrapper.getResultType())
                    && result.getResultCustomType().equals(wrapper.getResultCustomType()))) {
                ...
                setCommand = new SetCommand(editingDomain, result, EsbPackage.Literals.RULE_RESULT__RESULT_TYPE,
                        resultType);
                getResultCommand().append(setCommand);
            }
            ...
            if (!result.getValueLiteral().equals(wrapper.getValueLiteral())) {
                setCommand = new SetCommand(editingDomain, result, EsbPackage.Literals.RULE_RESULT__VALUE_LITERAL,
                        wrapper.getValueLiteral());
                getResultCommand().append(setCommand);
            }

            if (!result.getValueExpression().equals(wrapper.getValueExpression())) {
                setCommand = new SetCommand(editingDomain, result, EsbPackage.Literals.RULE_RESULT__VALUE_EXPRESSION,
                        wrapper.getValueExpression());
                getResultCommand().append(setCommand);
            }

            if (!result.getValueKey().equals(wrapper.getValueKey())) {
                setCommand = new SetCommand(editingDomain, result, EsbPackage.Literals.RULE_RESULT__VALUE_KEY,
                        wrapper.getValueKey());
                getResultCommand().append(setCommand);
            }
        ...
        if (getResultCommand().canExecute()) {
            ...
            if (getResultCommand().getCommandList().size() > 1) {
                log.error("RuleMediator results configuration : cannot save results",
