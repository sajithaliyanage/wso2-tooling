 ...
 */
public class ExpressionPropertyEditorDialog extends Dialog {
    /**
     * Dialog shell.
     */
    ...
    /**
     * Group box for separating json path and xpath selection area.
     */
    private Group expressionTypeSelectionGroupBox;

    /**
     * Combo box select expression type from json path and xpath.
     */
    private Combo expressionTypeSelectionComboDropDown;

    /**
     * Xpath or JSON path selection label.
     */
    private Label expressionTypeSelectionLabel;

    /**
     * Group box for text field for JSON path.
     */
    private Group jsonPathGroupBox;

    /**
     * Text box input for JSON path.
     */
    private Text jsonPathTextField;
    ...
    private String JSON_PATH = "JSON Path";
     ...
     */
    public ExpressionPropertyEditorDialog(Shell parent, int style, NamespacedProperty property) {
        super(parent, style);
    ...
    public static void main(String[] args) {
        ...
        ExpressionPropertyEditorDialog dialog = new ExpressionPropertyEditorDialog(shell, SWT.NULL,
    ...
    public void open() {
        ...
        // Configure expression type selection group box and child elements
        expressionTypeSelectionGroupBox = new Group(dialogShell, SWT.NONE);
        {
            expressionTypeSelectionGroupBox.setText("Expression Type");
            FormData groupLayoutData = new FormData();
            groupLayoutData.top = new FormAttachment(0);
            groupLayoutData.left = new FormAttachment(0);
            groupLayoutData.right = new FormAttachment(100);
            expressionTypeSelectionGroupBox.setLayoutData(groupLayoutData);

            // Configure group box internal layout.
            FormLayout xpathAndJsonBoxLayout = new FormLayout();
            xpathAndJsonBoxLayout.marginWidth = 5;
            xpathAndJsonBoxLayout.marginHeight = 5;
            expressionTypeSelectionGroupBox.setLayout(xpathAndJsonBoxLayout);

            // Configure expression type selection label
            expressionTypeSelectionLabel = new Label(expressionTypeSelectionGroupBox, SWT.NONE);
            expressionTypeSelectionLabel.setText("Select the Expression type:");
            FormData labelLayoutData = new FormData();
            labelLayoutData.top = new FormAttachment(0);
            labelLayoutData.left = new FormAttachment(0);
            expressionTypeSelectionGroupBox.setLayoutData(labelLayoutData);

            // Configure expression type selection combo box
            expressionTypeSelectionComboDropDown = new Combo(expressionTypeSelectionGroupBox,
                    SWT.DROP_DOWN | SWT.BORDER | SWT.READ_ONLY);
            expressionTypeSelectionComboDropDown.add(XPATH);
            expressionTypeSelectionComboDropDown.add(JSON_PATH);
            // Select XPath as the default option
            expressionTypeSelectionComboDropDown.select(0);

            // Format the combo box layout
            FormData comnboButtonLayoutData = new FormData();
            comnboButtonLayoutData.left = new FormAttachment(expressionTypeSelectionLabel, 15);
            comnboButtonLayoutData.right = new FormAttachment(100);
            comnboButtonLayoutData.top = new FormAttachment(-15);
            expressionTypeSelectionComboDropDown.setLayoutData(comnboButtonLayoutData);

        }

        // Construct and layout JSON path edit box.
        jsonPathGroupBox = new Group(dialogShell, SWT.NONE);
        {
            jsonPathGroupBox.setText("JSON Path");
            FormData jsonPathBoxLayoutData = new FormData();
            jsonPathBoxLayoutData.top = new FormAttachment(expressionTypeSelectionGroupBox, 5);
            jsonPathBoxLayoutData.left = new FormAttachment(0);
            jsonPathBoxLayoutData.right = new FormAttachment(100);
            jsonPathGroupBox.setLayoutData(jsonPathBoxLayoutData);

            // Configure group box internal layout.
            FormLayout groupBoxLayout = new FormLayout();
            groupBoxLayout.marginWidth = 5;
            groupBoxLayout.marginHeight = 5;
            jsonPathGroupBox.setLayout(groupBoxLayout);

            // JSON path editor text field.
            jsonPathTextField = new Text(jsonPathGroupBox, SWT.BORDER);
            FormData textFieldLayoutData = new FormData();
            textFieldLayoutData.right = new FormAttachment(100);
            textFieldLayoutData.top = new FormAttachment(0);
            textFieldLayoutData.left = new FormAttachment(0);
            jsonPathTextField.setLayoutData(textFieldLayoutData);
        }
        // This implementation selects XPath as the default value, so it is need to hide the json path group box
        jsonPathGroupBox.setVisible(false);
        ...
        propertyGroupBox = new Group(dialogShell, SWT.NONE);
        {
            ...
            groupBoxLayoutData.top = new FormAttachment(expressionTypeSelectionGroupBox, 5);
    ...
    private void loadConfiguration() {
        dialogShell.setText(String.format("Expression Property Editor", nsProperty.getPrettyName()));
        propertyGroupBox.setText(nsProperty.getPrettyName());

        if (!StringUtils.isBlank(nsProperty.getPropertyValue())) {
            // IF the existing property is JSON path expression this selects the JSON path as the expression type
            if (nsProperty.getPropertyValue().contains("json-eval")) {
                jsonPathTextField.setText(nsProperty.getPropertyValue());
                expressionTypeSelectionComboDropDown.select(1);
                showJsonPathBox();
            } else {
    ...
    private void initActions() {
        ...
        expressionTypeSelectionComboDropDown.addListener(SWT.Selection, new Listener() {
            public void handleEvent(Event event) {
                if (expressionTypeSelectionComboDropDown.getText().equals(XPATH)) {
                    showXPathBox();
                } else if (expressionTypeSelectionComboDropDown.getText().equals(JSON_PATH)) {
                    showJsonPathBox();
                }
    ...
    private void saveConfiguration() throws Exception {
        if (expressionTypeSelectionComboDropDown.getText().equals(JSON_PATH)) {
            // If the user saved value is a JSON path this will save the JSON path from the corresponding text field
            nsProperty.setPropertyValue(jsonPathTextField.getText());
    ...
    public void showJsonPathBox() {
        jsonPathGroupBox.setVisible(true);
        namespacesGroupBox.setVisible(false);
        propertyGroupBox.setVisible(false);
    }

    public void showXPathBox() {
        jsonPathGroupBox.setVisible(false);
        namespacesGroupBox.setVisible(true);
        propertyGroupBox.setVisible(true);
    }
}
