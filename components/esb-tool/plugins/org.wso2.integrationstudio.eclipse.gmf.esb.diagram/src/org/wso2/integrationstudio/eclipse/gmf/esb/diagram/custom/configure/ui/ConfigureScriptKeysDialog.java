...
public class ConfigureScriptKeysDialog extends TitleAreaDialog {
    ...
    public ConfigureScriptKeysDialog(Shell parentShell, ScriptMediator scriptMediator,
    ...
    private void unbindKeys(int itemIndex) {
        ...
        if (null != regProperty.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, scriptMediator,
                    EsbPackage.Literals.SCRIPT_MEDIATOR__SCRIPT_KEYS, regProperty);
    ...
    protected void okPressed() {
        for (TableItem item : tblKeys.getItems()) {
            ...
            if (null == property.eContainer()) {
                addCmd = new AddCommand(editingDomain, scriptMediator, EsbPackage.Literals.SCRIPT_MEDIATOR__SCRIPT_KEYS,
                        property);
                getResultCommand().append(addCmd);
            }
            SetCommand setCommand = null;
            if (!property.getKeyValue().equals(item.getText())) {
                setCommand = new SetCommand(editingDomain, property, EsbPackage.Literals.RULE_FACT__FACT_NAME,
                        item.getText());
        ...
        if (getResultCommand().canExecute()) {
            ...
            if (getResultCommand().getCommandList().size() > 1) {
                log.error(" ScriptMediator Keys configuration : cannot save keys",
