...
import static org.wso2.integrationstudio.eclipse.gmf.esb.diagram.debugger.utils.ESBDebuggerConstants.ESB_SKIPPOINT_ACTION_ID;
import static org.wso2.integrationstudio.eclipse.gmf.esb.diagram.debugger.utils.ESBDebuggerConstants.ESB_SKIPPOINT_COMMAND_LABEL;
import static org.wso2.integrationstudio.eclipse.gmf.esb.diagram.debugger.utils.ESBDebuggerConstants.ESB_SKIPPOINT_COMMAND_TOOL_TIP;
...
import org.wso2.integrationstudio.eclipse.gmf.esb.diagram.custom.configure.ConfigureEsbNodeAction;
...
/**
 * A {@link AbstractActionHandler} used to hook-up action for set and clear skip
 * points in esb design view editor.
 */
public class ESBSkipPointAction extends ConfigureEsbNodeAction {
    ...
    public ESBSkipPointAction(IWorkbenchPart part) {
        super(part);
        super.init();
        setId(ESB_SKIPPOINT_ACTION_ID);
        setText(ESB_SKIPPOINT_COMMAND_LABEL);
        setToolTipText(ESB_SKIPPOINT_COMMAND_TOOL_TIP);
    ...
    /**
     * This method performs the action when click the menu item Toggle Skip
     * Point
     */
    @Override
    protected void doRun(IProgressMonitor progressMonitor) {
        ...
        if (selectedEP instanceof AbstractMediator && ESBDebugPointTarget.canToggleDiagramDebugpoints(selectedEP)) {
            try {
                ESBDebugPointTarget.toggleDiagramDebugpoints((AbstractMediator) selectedEP, SKIP_POINT_LABEL);
            } catch (CoreException | ESBDebuggerException e) {
                log.error("Error while registering the skip point" + e.getMessage(), e);
            }
        }
