...
import org.wso2.integrationstudio.eclipse.gmf.esb.RuleFactValueType;
...
public class ConfigureRuleMediatorFactsDialog extends TitleAreaDialog {
    ...
    public ConfigureRuleMediatorFactsDialog(Shell parentShell, RuleMediator ruleMediator,
    ...
    protected void editItem(final TableItem item) {
        ...
        nameEditor = initTableEditor(nameEditor, item.getParent());
    ...
    private void unbindFact(int itemIndex) {
        ...
        if (null != fact.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, ruleMediator.getFactsConfiguration(),
    ...
    protected void okPressed() {
        for (TableItem item : tblFacts.getItems()) {
            ...
            if (!fact.getFactName().equals(wrapper.getFactName())) {
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__FACT_NAME,
                        wrapper.getFactName());
                getResultCommand().append(setCommand);
            }

            if (!(fact.getFactType().equals(wrapper.getFactType())
                    && fact.getFactCustomType().equals(wrapper.getFactCustomType()))) {
                ...
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__FACT_CUSTOM_TYPE,
                        customFactType);
                getResultCommand().append(setCommand);

                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__FACT_TYPE, factType);
                getResultCommand().append(setCommand);
            }
            ...
            if (!fact.getValueType().equals(wrapper.getValueType())) {
                ...
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__VALUE_TYPE,
                        wrapper.getValueType());
                getResultCommand().append(setCommand);
            }

            if (!fact.getValueLiteral().equals(wrapper.getValueLiteral())) {
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__VALUE_LITERAL,
                        wrapper.getValueLiteral());
                getResultCommand().append(setCommand);
            }

            if (!fact.getValueExpression().equals(wrapper.getValueExpression())) {
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__VALUE_EXPRESSION,
                        wrapper.getValueExpression());
                getResultCommand().append(setCommand);
            }

            if (!fact.getValueKey().equals(wrapper.getValueKey())) {
                setCommand = new SetCommand(editingDomain, fact, EsbPackage.Literals.RULE_FACT__VALUE_KEY,
                        wrapper.getValueKey());
                getResultCommand().append(setCommand);
            }
        ...
        if (getResultCommand().canExecute()) {
            ...
            if (getResultCommand().getCommandList().size() > 1) {
                log.error("RuleMediator facts configuration : cannot save facts",
