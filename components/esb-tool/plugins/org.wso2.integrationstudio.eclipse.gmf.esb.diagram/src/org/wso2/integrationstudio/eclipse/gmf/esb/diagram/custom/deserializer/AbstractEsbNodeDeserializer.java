...
public abstract class AbstractEsbNodeDeserializer<T, R extends EsbNode> implements IEsbNodeDeserializer<T, R> {
     ...
     * @throws DeserializerException
     */
    protected void deserializeSequence(IGraphicalEditPart part, SequenceMediator sequenceMediator,
            EsbConnector connector, boolean reversed) throws DeserializerException {
        ...
        if (connector instanceof OutputConnector) {
            ...
            for (int i = 0; i < mediatorsCount; ++i) {
                ...
                if (reversedNodes.contains(connector.eContainer())) {
                    executeMediatorDeserializer(part, nodeList, domain, mediator, true, commentMediatorList,
                            lastMediator);
                    reversedNodes.addAll(nodeList);
                } else {
                    executeMediatorDeserializer(part, nodeList, domain, mediator, reversed, commentMediatorList,
                            lastMediator);
                    if (reversed) {
                        reversedNodes.addAll(nodeList);
                    }
                    ...
                    if (reversed) {
                        ...
                        if (lastMediator && !faultSequence) {
                            if (proxyEditPart != null) {
                                abstractOutputConnectorEditPart = EditorUtils
                                        .getProxyOutSequenceOutputConnector(proxyEditPart);
                            }
                            APIResourceEditPart apiEditPart = EditorUtils.getApiResourceFromEditPart(part);
                            if (apiEditPart != null) {
                                abstractOutputConnectorEditPart = EditorUtils
                                        .getProxyOutSequenceOutputConnector(apiEditPart);
                            }
    ...
    private static void pairMediatorFlows() {
        for (Map.Entry<EsbConnector, EsbConnector> pair : pairMediatorFlowMap.entrySet()) {
            ...
            if (firstPart instanceof SequencesOutputConnectorEditPart
                ...
                AbstractSequencesEditPart sequence = EditorUtils
                        .getSequence((SequencesOutputConnectorEditPart) firstPart);
                IGraphicalEditPart mediatorFlow = (IGraphicalEditPart) sequence.getChildren().get(0); // assuming
                                                                                                      // mediatorflow is
                                                                                                      // the first child
                ...
                if (numOfChildren >= 1) {
                    ...
                    ShapeNodeEditPart lastChild = (ShapeNodeEditPart) mediatorFlowCompartment.getChildren()
                            .get(numOfChildren - 1);
                    AbstractOutputConnectorEditPart lastOutputConnector = EditorUtils
                            .getMediatorOutputConnector(lastChild);
                ...
                if (seq != null && seq.size() > 0 && seq.getLast() instanceof Sequence) {
                    sourceConnector = (AbstractConnectorEditPart) getEditpart(
                            ((Sequence) seq.getLast()).getOutputConnector().get(0));
                    targetConnector = (AbstractConnectorEditPart) secondPart;
                }
            } else {
                if (firstPart instanceof ProxyOutputConnectorEditPart
                    ...
                    if (seq != null && seq.size() > 0 && seq.getLast() instanceof SendMediator) {
                        sourceConnector = (AbstractConnectorEditPart) getEditpart(
                                ((SendMediator) seq.getLast()).getOutputConnector());
                    ...
                    if (seq != null && seq.size() > 0 && seq.getLast() instanceof SendMediator) {
                        ...
                        EList<APIResourceInSequenceInputConnector> inputConnectors = ((APIResource) ((Node) EditorUtils
                                .getAPIResource((AbstractConnectorEditPart) secondPart).getModel()).getElement())
            ...
            if (editpart instanceof complexFiguredAbstractMediator) {
                ...
                for (Object child : children) {
                    if (child instanceof AbstractOutputConnectorEditPart) {
                        EsbConnector connector = (EsbConnector) ((Node) ((EditPart) child).getModel()).getElement();
    ...
    private static void clearLinks() {
        ...
        while (iterator.hasNext()) {
            ...
            if (editPart instanceof EsbLinkEditPart) {
                ...
                /*
                 * We shouldn't remove EsbLinkEditParts if the target of the link is a
                 * AbstractEndpointInputConnectorEditPart.
                 * Because these kind of links will not be get regenerated again according to the current
                 * implementation.
                 */
    ...
    protected boolean executeSetValueCommand(EObject elementToEdit, EStructuralFeature feature, Object value) {
        SetRequest reqSet = new SetRequest(getDiagramEditor().getEditingDomain(), elementToEdit, feature, value);
        SetValueCommand operation = new SetValueCommand(reqSet);
        if (operation.canExecute()) {
            getDiagramEditor().getDiagramEditDomain().getDiagramCommandStack().execute(new ICommandProxy(operation));
            return true;
        }
        return false;
    }
