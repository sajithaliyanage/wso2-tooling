        ...
        if (keyValue != null) {
            ...
            if (keyValue.getKeyValue() != null && !keyValue.getKeyValue().equals("")) {
                ...
                if (!executeSetValueCommand(FAST_XSLT_MEDIATOR__FAST_XSLT_SCHEMA_KEY_TYPE, KeyType.STATIC)) {
                    log.warn(
                            "executeSetValueCommand returned false in Fast XSLT mediator Deserializer for Schema key type");
                }
                ...
                if (!executeSetValueCommand(FAST_XSLT_MEDIATOR__FAST_XSLT_STATIC_SCHEMA_KEY, regKey)) {
                    log.warn(
                            "executeSetValueCommand returned false in Fast XSLT mediator Deserializer for Static Schema Key");
                }
                ...
                if (!executeSetValueCommand(FAST_XSLT_MEDIATOR__FAST_XSLT_DYNAMIC_SCHEMA_KEY, nsp)) {
                    log.warn(
                            "executeSetValueCommand returned false in Fast XSLT mediator Deserializer for Dynamic schema key");
                }
            }
        }
