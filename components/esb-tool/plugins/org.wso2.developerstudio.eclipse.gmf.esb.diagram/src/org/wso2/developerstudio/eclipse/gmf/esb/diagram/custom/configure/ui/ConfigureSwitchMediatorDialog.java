...
public class ConfigureSwitchMediatorDialog extends Dialog {
    ...
    private void unbindCase(int itemIndex) {
        ...
        if (null != s.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, mediator,
                    EsbPackage.Literals.SWITCH_MEDIATOR__CASE_BRANCHES, s);
    ...
    protected void okPressed() {
         ...
         * xPath,EsbPackage.Literals.NAMESPACED_PROPERTY__PROPERTY_VALUE, txtXPath.getText());
        ...
        for (TableItem item : tblCases.getItems()) {
            ...
            if (null == s.eContainer()) {
                ...
                AddCommand addCmd = new AddCommand(editingDomain, mediator,
                        EsbPackage.Literals.SWITCH_MEDIATOR__CASE_BRANCHES, s);
                getResultCommand().append(addCmd);
            } else {
                if (!s.getCaseRegex().equals(item.getText())) {
                    SetCommand setCmd = new SetCommand(editingDomain, s,
                            EsbPackage.Literals.SWITCH_CASE_BRANCH_OUTPUT_CONNECTOR__CASE_REGEX, item.getText(1));
                    getResultCommand().append(setCmd);
                }
            }
        }
